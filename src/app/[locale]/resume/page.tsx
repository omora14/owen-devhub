...
export default function ResumePage({ params: { locale } }: { params: { locale: string } }) {
  ...
  const openPdf = isEs ? 'Abrir PDF' : 'Open PDF';
  const mobileHelp = isEs
    ? 'Si la vista previa no carga en tu teléfono, abre el PDF directamente.'
    : 'If preview does not load on your phone, open the PDF directly.';
  ...
  return (
    <main className="min-h-screen bg-ocean-deep text-sand-warm">
      <div className="max-w-6xl mx-auto px-4 sm:px-8 lg:px-16 xl:px-24 pt-24 pb-10">
        ...
        <div className="md:hidden rounded-2xl border border-white/10 bg-ocean-navy/70 p-5 space-y-4">
          <p className="text-sm text-sand-warm/75">{mobileHelp}</p>
          <div className="flex flex-col gap-3">
            <a
              href="/Owen-Morales-Resume.pdf"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-sunset-orange text-white text-sm font-medium hover:opacity-90 transition-opacity"
            >
              {openPdf}
            </a>
            <a
        ...
        <div className="hidden md:block rounded-2xl overflow-hidden border border-white/10 bg-ocean-navy shadow-2xl shadow-black/50">
          <iframe
            title="Resume PDF"
